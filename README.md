<p align="center">
  <img src="./assets/desk-setup-banner.gif" alt="Keith Nguyen's day-to-night development desk setup" width="100%" />
</p>

<video src="https://github.com/user-attachments/assets/a003d885-3b5b-4d49-a899-d420ee26bd08"></video>

<p align="center"><sub>Desk soundtrack — press play, then unmute.</sub></p>

<h1 align="center">Tuan Kiet (Keith) Nguyen</h1>

<p align="center">
  <strong>Software developer focused on iOS, full-stack web, and backend development.</strong>
</p>

<p align="center">
  <img
    src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=21&pause=1100&color=58A6FF&center=true&vCenter=true&width=850&lines=SwiftUI+%C2%B7+React+%C2%B7+FastAPI+%C2%B7+Java;iOS+apps+%C2%B7+Full-stack+systems+%C2%B7+Backend"
    alt="Mobile, full-stack, and backend software developer"
  />
</p>

<p align="center">
  <a href="https://www.linkedin.com/in/kietng2303/">
    <img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin" alt="Connect with Keith Nguyen on LinkedIn" />
  </a>
  <a href="#featured-projects">
    <img src="https://img.shields.io/badge/Featured-Projects-238636?style=for-the-badge&logo=github" alt="View featured projects" />
  </a>
</p>

---

## About Me

I'm a software developer working across iOS, full-stack web, and backend development. My public projects include a location-based iOS platform, an animal-adoption management system, and a Java banking application.

- 📱 Building iOS applications with **SwiftUI, Firebase, Apple Maps, and MVVM**
- 🌐 Building web applications with **React, TypeScript, FastAPI, and PostgreSQL**
- ☕ Currently strengthening my **Java, OOP, and testing** skills

## Featured Projects

<table>
  <tr>
    <td width="50%" valign="top">
      <h3>💅 <a href="https://github.com/keithnguyen2303/NailConnect">NailConnect</a></h3>
      <p>A location-aware iOS marketplace that helps nail technicians find flexible work and salon owners fill staffing needs.</p>
      <p><strong>Engineering:</strong> two-sided role flows, matching, maps, async Firebase data, local persistence, and MVVM boundaries.</p>
      <p>
        <code>Swift</code> <code>SwiftUI</code> <code>Firebase</code> <code>SwiftData</code> <code>Apple Maps</code>
      </p>
      <p><a href="https://youtu.be/qCv3sMDpbr4"><strong>Watch the product demo →</strong></a></p>
    </td>
    <td width="50%" valign="top">
      <h3>🐾 <a href="https://github.com/keithnguyen2303/ACAMS-Project">ACAMS</a></h3>
      <p>A full-stack animal care and adoption system for managing animals, users, and adoption requests.</p>
      <p><strong>Engineering:</strong> role-based authorization, protected routes, lifecycle rules, relational data, and a Dockerized multi-service stack.</p>
      <p>
        <code>React</code> <code>TypeScript</code> <code>FastAPI</code> <code>PostgreSQL</code> <code>Docker</code>
      </p>
      <p><a href="https://github.com/keithnguyen2303/ACAMS-Project"><strong>Explore the architecture →</strong></a></p>
    </td>
  </tr>
  <tr>
    <td colspan="2" valign="top">
      <h3>🏦 <a href="https://github.com/keithnguyen2303/java-bank-cli">Java Bank CLI</a></h3>
      <p>A compact banking engine built to make core Java concepts concrete: accounts, transaction history, inheritance, interfaces, streams, checked exceptions, and balance-sorted reporting.</p>
      <p><strong>Engineering:</strong> object-oriented domain modeling, collection-backed storage, executable packaging, and a JUnit 5 test suite.</p>
      <p><code>Java</code> <code>Maven</code> <code>JUnit 5</code> <code>OOP</code></p>
    </td>
  </tr>
</table>

## Tech Stack

<p align="center">
  <img
    src="https://skillicons.dev/icons?i=swift,java,py,ts,js,react,tailwind,fastapi,postgres,firebase,docker,git,github&perline=13"
    alt="Swift, Java, Python, TypeScript, JavaScript, React, Tailwind CSS, FastAPI, PostgreSQL, Firebase, Docker, Git, and GitHub"
  />
</p>

## GitHub Stats

<p align="center">
  <img
    height="165"
    src="https://github-readme-stats.vercel.app/api?username=keithnguyen2303&show_icons=true&hide_border=true&theme=transparent&rank_icon=github&include_all_commits=true"
    alt="Keith Nguyen's GitHub statistics"
  />
  <img
    height="165"
    src="https://github-readme-stats.vercel.app/api/top-langs/?username=keithnguyen2303&layout=compact&hide_border=true&theme=transparent&langs_count=8"
    alt="Keith Nguyen's most-used public repository languages"
  />
</p>

## Contributions

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/keithnguyen2303/keithnguyen2303/output/github-contribution-grid-snake-dark.svg" />
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/keithnguyen2303/keithnguyen2303/output/github-contribution-grid-snake.svg" />
  <img alt="Animated contribution graph" src="https://raw.githubusercontent.com/keithnguyen2303/keithnguyen2303/output/github-contribution-grid-snake.svg" />
</picture>
